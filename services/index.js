...
const createUser = async ({ email, password }) => {
  try {
    ...
    const codUnicDeVerificare = uuidv4();
      const linkDeVerificare = `http://localhost:3000/api/users/verify/${encodeURIComponent(
        codUnicDeVerificare
      )}`;
...
const sendVerificationEmail = async (email, verificationToken) => {
  const linkDeVerificare = `http://localhost:3000/api/users/verify/${verificationToken}`;
  const mg = mailgun.client({
    username: "api",
    key: process.env.MAILGUN_API_KEY,
  });
